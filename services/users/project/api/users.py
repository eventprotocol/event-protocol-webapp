...
#
# # address = '0x0e35462535dae6fd521f0eea67dc4e9485c714dc'
# # api_key = 'RA5HQDQNQXD9V1FK6ZTEJYWDGYWPAEPURC'
# # contract_address = '0x7143a8faa78b56fbdfefe0cfba58016f21620bf6'
#
# def getContractTransactions(address, contract_address, apiKey):
#     df = pd.DataFrame(columns = ['timeStamp', 'to', 'value', 'isError', 'txreceipt_status'])
#     with urllib.request.urlopen("http://api-rinkeby.etherscan.io/api?module=account&action=txlist&address={}&startblock=0&endblock=99999999&sort=asc&apikey={}".format(address, apiKey)) as url:
#         data = json.loads(url.read().decode())
#         #print(data['result'])
#
#     for i in range(0, len(data['result'])):
#         if (data['result'][i]['to'] == contract_address):
#             df = df.append({'timeStamp': data['result'][i]['timeStamp'],
#                             'to': data['result'][i]['to'],
#                             'value': data['result'][i]['value'],
#                             'isError': data['result'][i]['isError'],
#                             'txreceipt_status': data['result'][i]['txreceipt_status']},
#                     ignore_index=True)
#
#     return render_template('simple.html',  tables=[df.to_html(classes='data')], titles=df.columns.values)
#
#
# def getAllTransactions(address, apiKey):
#     df = pd.DataFrame(columns = ['timeStamp', 'to', 'value', 'isError', 'txreceipt_status'])
#     with urllib.request.urlopen("http://api-rinkeby.etherscan.io/api?module=account&action=txlist&address={}&startblock=0&endblock=99999999&sort=asc&apikey={}".format(address, apiKey)) as url:
#         data = json.loads(url.read().decode())
#         #print(data['result'])
